 ...
 * @LastEditors: qrmd
 * @LastEditTime: 2022-08-24 22:43:00
...
private:
    ...
    // 是否在待修补的位置附近有僵尸时不对其修补
    bool _is_safe_fix = false;
    // 指定咖啡豆修补仅对此植物有效
    int _be_woken_plant = -1;
    // 是否在附近有即将落锤的巨人僵尸或冰车僵尸时自动铲除南瓜头
    bool _is_auto_shovel_pumpkin = false;
    // 执行修补判断与操作的时间间隔（厘秒）
    int _run_interval = 1;
...
public:
    ...
    // *** In Queue
    // 设置是否在待修补的位置附近存在僵尸时不修补
    void setIsSafeFix(bool is_safe_fix);

    // *** In Queue
    // 指定咖啡豆修补仅对此植物有效，仅用于咖啡豆修补
    void setBeWokenPlant(int be_woken_plant);

    // *** In Queue
    // 设置是否在附近有即将落锤的巨人僵尸或冰车僵尸时自动铲除南瓜头，仅用于南瓜头修补
    void setIsAutoShovelPumpkin(bool is_auto_shovel_pumpkin);
...
void PlantFixerPlus::_Run()
{
    ...
    // 在附近有即将落锤的巨人僵尸或冰车僵尸时自动铲除南瓜头
    for (Grid each : _grids) {
        if (_is_auto_shovel_pumpkin) {
            ...
            if (GetPlantIndex(each.row, each.col, PUMPKIN) >= 0) {
                for (int index = 0; index < GetMainObject()->zombieTotal(); ++index) {
                    if (!zombies[index].isDisappeared() && !zombies[index].isDead() && zombies[index].row() + 1 == each.row) {
                        if ((zombies[index].abscissa() >= each.col * 80 - 101 && zombies[index].abscissa() <= each.col * 80 + 71 && zombies[index].isHammering() && animations[zombies[index].animationCode()].circulationRate() >= 0.63) || (zombies[index].type() == ZOMBONI && zombies[index].abscissa() <= each.col * 80 + 32)) {
                            _ShovelThePlant(each.row, each.col, PUMPKIN);
                            return;
                        }
                    }
                }
    ...
    for (Grid each_min_hp_grid : min_hp_grids) {
        ...
        // 在待修补的位置附近有僵尸时不对其修补
        bool is_nearby_exist_zombie;
        if (_is_safe_fix) {
            is_nearby_exist_zombie = false;
            for (auto&& zombie : alive_zombie_filter) {
                if (zombie->row() + 1 == each_min_hp_grid.row && zombie->abscissa() >= each_min_hp_grid.col * 80 - 101 && zombie->abscissa() <= each_min_hp_grid.col * 80 + 120) {
                    is_nearby_exist_zombie = true;
                    break;
                }
            }
            if (is_nearby_exist_zombie)
                continue;
        }
        // 仅对指定的植物使用咖啡豆
        if (_be_woken_plant != -1) {
            if (GetPlantIndex(each_min_hp_grid.row, each_min_hp_grid.col, _be_woken_plant) < 0)
                continue;
        }
...
void PlantFixerPlus::setIsSafeFix(bool is_safe_fix)
{
    InsertOperation([=]() { _is_safe_fix = is_safe_fix; }, "setIsSafeFix:" + std::to_string(is_safe_fix));
}
void PlantFixerPlus::setBeWokenPlant(int be_woken_plant)
{
    InsertOperation([=]() {
        if (_type != COFFEE_BEAN) {
            ShowErrorNotInQueue("PlantFixerPlus:\n    指定咖啡豆修补仅对此植物有效仅用于咖啡豆修补", _type);
            return;
        }
        _be_woken_plant = be_woken_plant;
    },
        "setBeWokenPlant:" + std::to_string(be_woken_plant));
}

void PlantFixerPlus::setIsAutoShovelPumpkin(bool is_auto_shovel_pumpkin)
{

    InsertOperation([=]() {
        if (_type != PUMPKIN) {
            ShowErrorNotInQueue("PlantFixerPlus:\n    设置在附近有即将落锤的巨人僵尸或冰车僵尸时自动铲除南瓜头仅用于南瓜头修补", _type);
            return;
        }
        _is_auto_shovel_pumpkin = is_auto_shovel_pumpkin;
    },
        "setIsAutoShovelPumpkin:" + std::to_string(is_auto_shovel_pumpkin));
}
