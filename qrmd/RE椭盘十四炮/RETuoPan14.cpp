/*
 * @Author: qrmd
 * @Date: 2022-08-11 01:35:20
 * @LastEditors: qrmd
 * @LastEditTime: 2022-08-11 16:19:05
 * @Description:FE纯三炮 全难度极限出怪运阵演示脚本
...
void Script()
{
    ...
    // 选择植物：寒冰菇、模仿寒冰菇、毁灭菇、睡莲叶、樱桃炸弹、火爆辣椒、倭瓜、小喷菇、大蒜、高坚果
    SelectCards({ICE_SHROOM, M_ICE_SHROOM, DOOM_SHROOM, LILY_PAD, CHERRY_BOMB, JALAPENO, SQUASH, PUFF_SHROOM, GARLIC, TALL_NUT});
    SetTime(-599, 1);
    // 填充炮列表
    ...
    Card(GARLIC, 2, 4);
    // 主体节奏 C2 I-p-Nd|I'-p-PA'A 2501|2501 每波执行确定的操作，不考虑非旗帜波的刷新延迟

    for (auto wv : {1, 3, 4, 7, 9, 10, 12, 14, 16, 18, 20}) {
        // wavewv I-p-Nd
        SetTime(1 - 100, wv);
        Card(ICE_SHROOM, 2, 1);
        Card();
