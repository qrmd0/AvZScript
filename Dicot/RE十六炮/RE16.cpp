...
void AScript()
{
    ...
    {
        Connect(-10, aCobManager.RoofFire({{2, 9}, {2, 9}, {4, 9}}); // -10 + 387 < 377
                Delay(110, aCobManager.RoofFire(2, 8.8)));           // 110 拦截
        Connect(601 + 50 - 298, aIceFiller.Coffee());                // 50cs 预判冰
        if (wave == 9)
        {
            Connect(601 - 150, aCobManager.RoofFire(2, 9));
            Connect(601 + 1200 - 200 - 387, aCobManager.RoofFire({{5, 9}, {5, 9}});
    ...
    {
        Connect(-150, aCobManager.RoofFire(2, 9));
        Connect(1200 - 200 - 387, aCobManager.RoofFire({{2, 9}, {4, 9}}); // 1200cs 波长; 激活炸
                Delay(220, aCobManager.RoofFire(2, 7.8)));                // 220 拦截
        if (wave == 19)
        {
            Connect(1200 - 10, aCobManager.RoofFire({{2, 9}, {2, 9}, {4, 9}});
                    Delay(110, aCobManager.RoofFire(2, 8.8))); // 110 拦截
            Connect(1200 + 601 - 150, aCobManager.RoofFire(5, 9));
            Connect(1200 + 601 + 1200 - 200 - 387, // 等会儿
    ...
    for (auto wave : {20})
    {
        Connect(-200, aIceFiller.Coffee());                        // 冰消空降
        Connect(-100, aCobManager.RoofFire({{2, 8.5}, {5, 8.5}})); // 炸冰车
        Connect(50, aCobManager.RoofFire({{4, 2.5}, {4, 6.7}}));   // 炸小偷
